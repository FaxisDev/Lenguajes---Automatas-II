...
float sensor(int8 primero, int8 segundo) // funci�n que controla al sensor de temperatura parametros primero y segunto tipo entero de 8 bits
{
    while (true) // Cliclo siempre activo
    {
        ...
        if (conversion > conversion1) // Condisi�n conversi�n> conversi� 1
        {
            ...
            lcd_gotoxy(2, 1);                     // posici�n 2 en x y 1 en y de LCD
        ...
        if (conversion1 > conversion) // conversion1>conversion
        {
            ...
            temo = temp * 500 / 1023;
            lcd_gotoxy(2, 1);                      // posici�n 2x 1y del LCD
            printf(lcd_putc, "Tempera:-%f", temo); // impirme en LCD
            lcd_gotoxy(2, 2);                      // posici�n 2x 2y del LCD
...
void main()
{
    ...
    setup_adc(ADC_CLOCK_DIV_32);                // EL RELOJ DE CONVERSI�N  del ADC
    setup_adc_ports(sAN0 | sAN1 | sAN5 | sAN6); // Canales  anal�gicos a usarse AN0, AN1, AN5,AN6
    ...
    while (true) // bucle infinito
    {
        ...
        if (input(PIN_A2) == 1) // si la entrada A2 se presiona
        {
            ...
            printf(lcd_putc, "\f");       // borrando LCD
            delay_ms(500);
            sensor(5, 6);  // activa funci�n sensor con canala anal�gico 5, 6
        ...
        if (input(PIN_A3) == 1) // si la entrada A3 se presiona
        {
            ...
            printf(lcd_putc, "\f");       // borrando LCD
            delay_ms(500);
            sensor(0, 1); // activa funci�n sensor con canala anal�gico 0, 1
        }
    }
}